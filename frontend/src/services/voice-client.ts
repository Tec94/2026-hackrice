...
export function pcmToFloat(bytes: Uint8Array): Float32Array {
  if (!bytes.length || bytes.length % 2)
    throw new Error("Incomplete PCM audio.");
...
export class VoiceClient {
  ...
  async start(capture: () => Promise<ChartContext>) {
    ...
    if (!navigator.mediaDevices?.getUserMedia || !window.AudioContext) {
      this.callbacks.error(
        "Microphone capture needs a supported browser on localhost or HTTPS.",
      );
      return;
    }
    ...
    try {
        ...
        messages = messages
          .then(async () => {
            ...
            if (event.data instanceof ArrayBuffer) {
              ...
              if (!turn.approved || !turn.audioStarted || !turn.outputRate)
                throw new Error("Unapproved voice output was blocked.");
      ...
      socket.onerror = () =>
        this.fail(
          turn,
          new Error(
            "Could not connect to voice. Check the API and WebSocket proxy.",
          ),
        );
      socket.onclose = () => {
        if (this.turn === turn)
          this.fail(
            turn,
            new Error("Voice disconnected. Start a new turn to retry."),
          );
      };
    } catch (error) {
  ...
  private async event(turn: Turn, message: ServerMessage) {
    if (
      ...
      message.sequence <= turn.lastSequence
    )
      return;
    ...
    if (message.type === "session.ready") {
        ...
        try {
          ...
          while (turn.pendingSamples.length >= turn.frameSamples) {
            ...
            turn.socket.send(
              encodeAudioFrame(
                turn.id,
                floatToPcm(Float32Array.from(samples)),
              ).slice().buffer,
            );
          }
        } catch (error) {
      ...
      this.send(turn, {
        type: "voice.start",
        turnId: turn.id,
        chartSnapshotId: turn.snapshotId,
        format: message.inputFormat,
      });
      return;
    }
    if (
      ...
      throw new Error(
        `Voice unavailable (${message.error.code}). Check provider setup and playback validation.`,
      );
    }
    ...
    if (
      "chartSnapshotId" in message &&
      message.chartSnapshotId !== turn.snapshotId
    )
      throw new Error("Voice snapshot mismatch.");
    if (message.type === "voice.ready") {
      turn.listening = true;
      this.callbacks.state("listening");
    }
    if (message.type === "voice.transcript.final") {
    ...
    if (message.type === "assistant.response") {
      turn.approved = true;
      this.callbacks.answer(renderSafeReply(message.reply));
    }
    if (message.type === "assistant.audio.start") {
      if (!turn.approved)
        throw new Error("Unapproved voice output was blocked.");
      turn.audioStarted = true;
      turn.outputRate = message.format.sampleRateHz;
    }
    if (message.type === "assistant.completed") {
      turn.generated = true;
      this.finishPlayback(turn);
    }
  ...
  stop() {
    ...
    try {
      if (
        turn.pendingSamples.length &&
        turn.socket?.readyState === WebSocket.OPEN
      ) {
        turn.socket.send(
          encodeAudioFrame(
            turn.id,
            floatToPcm(Float32Array.from(turn.pendingSamples)),
          ).slice().buffer,
        );
        turn.pendingSamples = [];
      }
      this.stopMicrophone(turn);
      this.send(turn, { type: "voice.stop", turnId: turn.id });
      this.callbacks.state("thinking");
    } catch (error) {
      this.fail(turn, error);
    }
  ...
  private captureWorklet(context: AudioContext): Promise<void> {
    return (this.worklet ??=
      context.audioWorklet.addModule("/audio-capture.js"));
  }
  ...
  private stopMicrophone(turn: Turn) {
    ...
    turn.input?.disconnect();
    turn.worklet?.disconnect();
    if (turn.worklet) {
      turn.worklet.port.onmessage = null;
      turn.worklet.port.close();
    }
  }
  ...
  cancel() {
    ...
    if (turn.sentStart && turn.socket?.readyState === WebSocket.OPEN) {
      try {
        this.send(turn, { type: "voice.cancel", turnId: turn.id });
      } catch {
  ...
  private fail(turn: Turn, error: unknown) {
    ...
    this.close();
    this.callbacks.error(
      error instanceof Error ? error.message : "Voice failed. Try again.",
    );
  }
  ...
  private dispose(turn: Turn) {
    ...
    turn.sources.clear();
    if (turn.socket) {
      turn.socket.onclose = null;
      turn.socket.onerror = null;
      turn.socket.onmessage = null;
      turn.socket.close();
    }
